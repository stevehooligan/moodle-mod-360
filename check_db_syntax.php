...
// List of patterns to search.
    ...
    $helper = array (
        ...
        'sql_(as|bitand|bitnot|bitor|bitxor|cast_char2int|ceil|
            compare_text|concat|concat_join|empty|fullname|ilike|isempty|isnotempty|
            length|max|null_from_clause|order_by_text|paging_limit|position|substr)'
    );
...
// Define some known false positives to take them out from
// ...errors report (nested array of => file => regular expressions considered false positives).
    $fp = array (
          'install.php' => array(
...
// List of exceptions that aren't errors (function declarations, comments,.
// ...adodb usage from adodb drivers and harcoded strings). Non reportable false positives.
    $excludes = '/(function |^\s*\*|^\s*\/\/|\$this-\>adodb-\>(Execute|
        Connect|PConnect|ErrorMsg|MetaTables|MetaIndexes|MetaColumns|MetaColumnNames|MetaPrimaryKeys|)|protected \$[a-zA-Z]*db|
        Incorrect |check find_index_name|not available anymore|output|
        ...
        hint_database = install_db_val|Current database \(|
        admin_setting_configselect|(if|while|for|return).*\>get_recordset(_list|
        _select|_sql)?.*\>valid\(\)|NEWNAMEGOESHERE.*XMLDB_LINEFEED|has_capability\(.*:view.*context)|
        die(.*result.*:null.*errstr)|CAST\(.+AS\s+(INT|FLOAT|DECIMAL|NUM|REAL)/';
...
function files_to_check($path) {
    ...
    while (false !== ($file=readdir($dir))) {
        ...
        if (is_file($fullpath) && (strpos($fullpath, 'lib/adodb')!==false ||
            ...
            // Exclude adodb, pear, simpletest, htmlpurifier,
            // ...memcached, phpmailer, soap and lucene libs, lang and config files.
            continue;
        }

        if (is_file($fullpath) && strpos($file, '.php')===false &&
            strpos($file, '.html')===false && strpos($file, '.xml')===false) { // Exclude some files.
            continue;
        }
